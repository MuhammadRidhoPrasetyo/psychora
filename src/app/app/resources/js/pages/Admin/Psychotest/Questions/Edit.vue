...
import type { PsychotestForm, PsychotestAspect, PsychotestQuestion } from '@/types/models';
import { Plus, Trash2 } from 'lucide-vue-next';

const props = defineProps<{ form: PsychotestForm; question: PsychotestQuestion; aspects: PsychotestAspect[] }>();

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Admin', href: '/admin' },
    { title: 'Psychotest', href: '#' },
    { title: props.form.name, href: `/admin/psychotest/forms/${props.form.id}` },
    { title: `Edit Q${props.question.question_number}`, href: '#' },
];

type OptionEntry = {
    id?: string; label: string; statement: string; sort_order: number;
    mappings: { id?: string; psychotest_aspect_id: string; psychotest_characteristic_id: string; weight: number }[];
};
...
const qForm = useForm({
    ...
    options: (props.question.options ?? []).map(o => ({
        ...
        mappings: (o.mappings ?? []).map(m => ({
            id: m.id,
            psychotest_aspect_id: m.psychotest_aspect_id,
            psychotest_characteristic_id: m.psychotest_characteristic_id,
            weight: m.weight,
        })),
    })) as OptionEntry[],
});
...
function submit() {
    qForm.put(`/admin/psychotest/forms/${props.form.id}/questions/${props.question.id}`);
}
</script>

<template>
    <Head :title="`Edit Q${question.question_number}`" />
    <AdminLayout :breadcrumbs="breadcrumbs">
        <Card class="max-w-4xl">
            <CardHeader><CardTitle>Edit Question #{{ question.question_number }}</CardTitle></CardHeader>
            <CardContent>
                <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div v-for="(opt, oi) in qForm.options" :key="oi" class="border rounded-lg p-4 space-y-3">
                            ...
                            <div class="ml-4 space-y-2">
                                ...
                                <div v-for="(m, mi) in opt.mappings" :key="mi" class="flex gap-2 items-end">
                                    ...
                                    <div class="grid gap-1 w-20">
                                        <Input type="number" v-model.number="m.weight" />
                                    </div>
                    ...
                    <div class="flex gap-2">
                        <Button type="submit" :disabled="qForm.processing">Save Changes</Button>
