...
import type { IstForm, Test } from '@/types/models';

const props = defineProps<{ form: IstForm; tests: Test[] }>();
...
function submit() {
    formData.put(`/admin/ist/forms/${props.form.id}`);
}
</script>

<template>
    <Head title="Edit IST Form" />
    <AdminLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto max-w-2xl">
            <Card>
                <CardHeader><CardTitle>Edit IST Form</CardTitle></CardHeader>
                <CardContent>
                    <form @submit.prevent="submit" class="space-y-4">
                        <div class="grid gap-2">
                            <Label>Test</Label>
                            <Select v-model="formData.test_id">
                                <SelectTrigger><SelectValue placeholder="Select IST test" /></SelectTrigger>
                                <SelectContent>
