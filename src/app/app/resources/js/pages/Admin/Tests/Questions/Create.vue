...
const props = defineProps<{ test: Test; sections: TestSection[] }>();
...
const form = useForm({
    ...
    score: 1,
    sort_order: 0,
    options: [] as { label: string; content: string; is_correct: boolean }[],
    essay_answers: [] as { answer_text: string; score: number }[],
});
...
function addEssayAnswer() {
    form.essay_answers.push({ answer_text: '', score: 1 });
}
function removeEssayAnswer(i: number) {
    form.essay_answers.splice(i, 1);
}

function submit() {
    form.post(`/admin/tests/${props.test.id}/questions`);
}
</script>
                    ...
                    <form @submit.prevent="submit" class="space-y-4">
                        ...
                        <div v-if="form.question_type === 'multiple_choice' || form.question_type === 'true_false'" class="space-y-3">
                            ...
                            <div v-for="(opt, i) in form.options" :key="i" class="flex gap-2 items-center">
                                <Input v-model="opt.label" class="w-16" placeholder="A" />
                                <Input v-model="opt.content" class="flex-1" placeholder="Option text" />
                        ...
                        <div v-if="form.question_type === 'essay'" class="space-y-3">
                            ...
                            <div v-for="(ans, i) in form.essay_answers" :key="i" class="flex gap-2 items-center">
                                <Input v-model="ans.answer_text" class="flex-1" placeholder="Answer text" />
                                <Input type="number" v-model.number="ans.score" class="w-20" placeholder="Score" />
