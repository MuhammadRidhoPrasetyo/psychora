...
import type { SubscriptionPlan, Program, TestType } from '@/types/models';

const props = defineProps<{ subscriptionPlan: SubscriptionPlan; programs: Program[]; testTypes: TestType[] }>();
...
const form = useForm({
    ...
    code: props.subscriptionPlan.code ?? '',
    description: props.subscriptionPlan.description ?? '',
    price: props.subscriptionPlan.price ?? 0,
    duration_days: props.subscriptionPlan.duration_days ?? 30,
    is_active: props.subscriptionPlan.is_active ?? true,
    sort_order: props.subscriptionPlan.sort_order ?? 0,
    entitlements: props.subscriptionPlan.entitlements?.map(e => ({
        program_id: e.program_id ?? '',
        test_type_id: e.test_type_id ?? '',
    })) ?? [],
});
...
function submit() {
    form.put(`/admin/subscription-plans/${props.subscriptionPlan.id}`);
}
</script>
...
<template>
    ...
    <AdminLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto max-w-2xl">
            <Card>
                <CardHeader><CardTitle>Edit Subscription Plan</CardTitle></CardHeader>
                <CardContent>
