...
const form = useForm({
    ...
    entitlements: props.plan?.entitlements?.map(e => ({
...
function removeEntitlement(index: number) {
    form.entitlements.splice(index, 1);
}

function submit() {
    if (isEdit) form.put(`/admin/subscription-plans/${props.plan!.id}`);
    else form.post('/admin/subscription-plans');
}
</script>
...
<template>
    ...
    <AdminLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto max-w-2xl">
            <Card>
                <CardHeader><CardTitle>{{ isEdit ? 'Edit Plan' : 'Create Plan' }}</CardTitle></CardHeader>
                <CardContent>
